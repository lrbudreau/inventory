...
export default function Orders({ currentUser }) {
  ...
  // Scrap state
  const [scrapModal, setScrapModal] = useState(null); // { item }
  ...
  async function printInvoice() {
    ...
    const itemRows = orderItems.map(item => {
      ...
      const price = item.price || getProductPrice(item.productID);
      const lineTotal = price * built;
      return `<tr>
    ...
    const subtotal = orderItems.reduce((s, item) => {
      const price = item.price || getProductPrice(item.productID);
      return s + price * (item.built || 0);
    }, 0);
    ...
    const html = `
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .total-row { text-align: right; font-weight: 700; font-size: 1rem; margin-bottom: 28px; padding-top: 8px; border-top: 2px solid #2d5a27; }
  ...
  function printOrder(summary = true) {
    ...
    const itemRows = orderItems.map(item => {
      ...
      return `<tr>
        <td>${getProductName(item.productID)}</td>
        <td>${item.quantity}</td>
        ${!summary ? `<td>${built}</td><td>${remaining}</td><td>$${price.toFixed(2)}</td>` : ""}
      </tr>`;
    }).join("");
    ...
    const html = `
      ...
      <table>
        <thead><tr>
          ...
          ${!summary ? "<th>Built</th><th>Remaining</th><th>Unit Price</th>" : ""}
        </tr></thead>
        <tbody>${itemRows}</tbody>
      </table>
  ...
  async function handleBuildForOrder(item) {
    ...
    const res = await apiPost("orderProducts/buildFor", {
      orderID: selected.id, productId: item.productID, count, newBuilt,
  ...
  async function handleUpdatePrice(item, newPrice) {
    await apiPost("orderProducts/updatePrice", {
      orderID: selected.id, productID: item.productID, price: newPrice
    });
    setOrderItems(orderItems.map(i =>
      i.productID == item.productID ? { ...i, price: Number(newPrice) } : i
    ));
  }
  ...
  async function handleScrap() {
    ...
    setScrapping(true);
    const res = await apiPost("orderProducts/scrap", {
      orderID: selected.id,
      productID: scrapModal.productID,
      quantity: scrapQty,
      reason: scrapReason,
    ...
    if (res.success) {
      ...
      showToast(res.error || "Scrap failed.", "fail");
    }
    setScrapping(false);
  }
  ...
  const orderTotal = (items) => items.reduce((s, i) => {
    const price = i.price || getProductPrice(i.productID);
  ...
  return (
    <>
      ...
      {/* Scrap Modal */}
      {scrapModal && (
        <div className="modal-overlay" onClick={() => setScrapModal(null)}>
            ...
            <div className="inline-form">
              ...
              <div className="field">
                ...
                <div className="qty-stepper" style={{justifyContent:"flex-start"}}>
                  <button className="stepper-btn" onClick={() => setScrapQty(q => Math.max(1, q - 1))}>−</button>
                  <span className="stepper-val">{scrapQty}</span>
                  <button className="stepper-btn" onClick={() => setScrapQty(q => q + 1)}>+</button>
                </div>
              </div>
              <div className="field">
                ...
                <input
                  ...
                  placeholder="e.g. Weld defect, wrong dimensions…"
                />
              </div>
              <div className="form-actions">
                <button className="btn-secondary" onClick={() => setScrapModal(null)}>Cancel</button>
                <button className="btn-danger" onClick={handleScrap} disabled={scrapping}>
                  {scrapping ? "Logging…" : "Log Scrap"}
                </button>
              </div>
            </div>
          </div>
        ...
        {view === "detail" && selected && (
          <div>
            <div className="order-meta card">
              ...
              <div className="order-meta-row">
                <span className="meta-label">Order Value</span>
                <span className="meta-value" style={{fontWeight:700, color:"var(--green)"}}>
                  ${orderTotal(orderItems).toFixed(2)}
                </span>
              </div>
              <div className="order-meta-row">
            ...
            <div className="card no-pad">
              <ul className="item-list">
                {orderItems.map(item => {
                  ...
                  const price = item.price || getProductPrice(item.productID);
                  const scrapped = item.scrapped || 0;
                  return (
                    <li key={item.productID} className="item-row" style={{flexDirection:"column", alignItems:"stretch", gap:6, padding:"12px 16px"}}>
                      <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start"}}>
                        <div className="item-main" style={{flex:1}}>
                          ...
                          <span className="item-sub">
                            ...
                            {scrapped > 0 && <span style={{color:"var(--danger)"}}> · {scrapped} scrapped</span>}
                          </span>
                          <div className="mini-progress-wrap">
                            <div className="mini-progress" style={{ width: `${Math.min(100, (built / item.quantity) * 100)}%` }} />
                          </div>
                        </div>
                        <div className="item-right" style={{alignItems:"flex-start", paddingTop:2}}>
                          {!done ? (
                      ...
                      {/* Price row + scrap */}
                      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8, paddingTop:4, borderTop:"1px solid var(--border)"}}>
                        <div style={{display:"flex", alignItems:"center", gap:8}}>
                          <span style={{fontSize:"0.75rem", color:"var(--text-muted)"}}>Unit price:</span>
                          <PriceEditor
                            value={price}
                            onSave={val => handleUpdatePrice(item, val)}
                          />
                          <span style={{fontSize:"0.75rem", color:"var(--text-muted)"}}>
                            · line: <strong>${(price * item.quantity).toFixed(2)}</strong>
                          </span>
                        </div>
                        <button
                          className="btn-scrap"
                          onClick={() => openScrapModal(item)}
                          title="Log a scrapped unit"
                        >
                          🗑 Scrap
                        </button>
                      </div>
...
function PriceEditor({ value, onSave }) {
    ...
    return (
      <div style={{display:"flex", gap:4, alignItems:"center"}}>
        <input
          ...
          autoFocus
          style={{width:80, padding:"2px 6px", fontSize:"0.85rem", border:"1px solid var(--border)", borderRadius:4, background:"var(--bg-input)", color:"var(--text)"}}
        />
        <button onClick={commit} style={{fontSize:"0.75rem", padding:"2px 6px", background:"var(--green)", color:"#fff", border:"none", borderRadius:4, cursor:"pointer"}}>✓</button>
        <button onClick={() => setEditing(false)} style={{fontSize:"0.75rem", padding:"2px 6px", background:"var(--border)", color:"var(--text)", border:"none", borderRadius:4, cursor:"pointer"}}>✕</button>
      </div>
    );
  }

  return (
    <button onClick={startEdit} style={{fontSize:"0.85rem", fontWeight:600, color:"var(--text)", background:"none", border:"1px dashed var(--border)", borderRadius:4, padding:"2px 8px", cursor:"pointer"}}>
